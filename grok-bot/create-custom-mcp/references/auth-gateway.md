...
gateway. It never goes in chat, never in a Connector URL, and never in
`AddMcpServer` `headers`. Stuffing `Authorization: Bearer <api-key>` into
the Connector copies the master key off the box into Grok’s config, where
it does not expire. That is a downgrade from this design.

How a Connector links:

1. Add the remote URL plus the route’s OAuth `CLIENT_ID` (public client,
   no secret). No API key in headers.
...
A Grok Connector is not that machine.
